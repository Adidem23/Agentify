...
export function SidebarDemoJIRA() {
  ...
  useEffect(() => {
    const fetchPreviousChats = async () => {
      try {
        const response = await axios.get("http://localhost:1302/api/getJIRAChatMetadata");
...
function ChatInterface() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:1302/api/getJIRASeverResponse", { query: input })
        ...
        <form onSubmit={handleSubmit} className="flex items-center space-x-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder="Ask JIRA Agent 🤖 a Task ..."
